...
class Tokenizer:
    ...
    KEYWORDS = {
        ...
        'UNIQUE', 'NOT', 'NULL', 'DEFAULT', 'AND', 'OR', 'JOIN', 'INNER',
        'LEFT', 'RIGHT', 'OUTER', 'ORDER', 'BY', 'ASC', 'DESC', 'LIMIT',
        'INT', 'VARCHAR', 'FLOAT', 'BOOLEAN', 'DATETIME'
    }
    ...
    def tokenize(self) -> List[Token]:
        ...
        while self.pos < len(self.query):
            ...
            # Comments
            if char == '-' and self.pos + 1 < len(self.query) and self.query[self.pos + 1] == '-':
                self._skip_comment()
                continue
...
class SQLParser:
    ...
    def _expect(self, expected: str, token_type: str = 'KEYWORD') -> Token:
        ...
        token = self._current_token()
        if not token or token.type != token_type or token.value.upper() != expected.upper():
            raise ValueError(f"Expected {expected}, got {token.value if token else 'EOF'}")
    ...
    def _parse_condition(self) -> Dict[str, Any]:
        ...
        while self._current_token() and self._current_token().value.upper() in ('AND', 'OR'):
